...
import flow.otp.HotpModule
import flow.otp.TotpModule
...
class OtpTest {
    ...
    fun `a base32 secret is the same secret`() {
        ...
        val fromBase32 = HotpModule().process(
    ...
    fun `a time-based code is a code of the right shape`() {
        val out = TotpModule().process(mapOf("secret" to secret), emptyMap())["out"]!!.decodeToString()
