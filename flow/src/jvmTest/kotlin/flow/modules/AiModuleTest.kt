...
class AiModuleTest {
    ...
    /** Runs the node with the Ollama address overridden the way the module resolves it. */
    private fun run(options: Map<String, String>, input: String, port: Int): String {
        ...
        try {
            val out = AiModule().process(mapOf("in" to input.encodeToByteArray()), options)
     ...
     * modules take Compose on — and that bargain is only kept if the worker is really started
    ...
    fun `the jar runs in a worker process, with the app's JSON on loan`() {
        val jar = java.io.File("../flow-modules/ai-module/build/libs/ai-module.jar")
            .let { if (it.isFile) it else java.io.File("flow-modules/ai-module/build/libs/ai-module.jar") }
        assertTrue(jar.isFile, "run :flow-modules:ai-module:jar first — ${jar.absolutePath}")

        val worker = flow.platform.ModuleProcess(jar.parentFile, listOf(jar))
        try {
            val described = worker.request(flow.module.host.Wire.DESCRIBE) {}
            ...
            val reply = worker.request(flow.module.host.Wire.PROCESS) { o ->
                flow.module.host.Wire.writeString(o, "flow.ai")
                flow.module.host.Wire.writeByteMap(o, mapOf("in" to "hello".encodeToByteArray()))
                flow.module.host.Wire.writeStringMap(o, mapOf("provider" to "openai", "model" to "m"))
                // the module's own settings, which every PROCESS carries — a sender that omits
                // them leaves the worker waiting on a pipe that will never say more
                flow.module.host.Wire.writeStringMap(o, emptyMap())
            }
    ...
    /**
     * An module's own settings reach it, underneath a node's options.
     *
     * Declared by the module, edited once in Settings ▸ Modules, and merged in by the worker
    ...
    fun `a blank node option takes the extension's setting, and a filled one does not`() {
        val jar = java.io.File("../flow-modules/ai-module/build/libs/ai-module.jar")
            .let { if (it.isFile) it else java.io.File("flow-modules/ai-module/build/libs/ai-module.jar") }
        assertTrue(jar.isFile, "run :flow-modules:ai-module:jar first")
        val worker = flow.platform.ModuleProcess(jar.parentFile, listOf(jar))
        try {
            // the module declares them, so the Settings screen has something to show
            val described = worker.request(flow.module.host.Wire.DESCRIBE) {}.payload.decodeToString()
    ...
    private fun ask(worker: flow.platform.ModuleProcess, node: String, setting: String): String {
        val reply = worker.request(flow.module.host.Wire.PROCESS) { o ->
            flow.module.host.Wire.writeString(o, "flow.ai")
            flow.module.host.Wire.writeByteMap(o, mapOf("in" to "x".encodeToByteArray()))
            flow.module.host.Wire.writeStringMap(o, mapOf("provider" to node))
            flow.module.host.Wire.writeStringMap(o, mapOf("provider" to setting))
        }
    ...
    fun `the settings offer a key, a model and a way to be reached`() {
        ...
        // breaks every module jar built before it (see ExtensionAbiTest)
    ...
    /** Points the module's config at a test server by writing the settings it reads. */
