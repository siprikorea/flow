...
class ViewExtensionTest {
    ...
    fun `a long line is wrapped to fit the width it was given`() {
        ...
        StringView().draw(canvas, "x".repeat(50).encodeToByteArray(), emptyMap())
        val columns = 16
    ...
    fun `wrapping can be turned off`() {
        ...
        StringView().draw(canvas, "x".repeat(50).encodeToByteArray(), mapOf("wrap" to "false"))
    ...
    fun `hex rows carry the offset the bytes are at`() {
        val canvas = FakeCanvas()
        HexView().draw(canvas, ByteArray(48) { it.toByte() }, mapOf("bytesPerRow" to "16"))
    ...
    fun `hex and characters describe the same bytes`() {
        val canvas = FakeCanvas()
        HexView().draw(canvas, "AB!".encodeToByteArray(), mapOf("bytesPerRow" to "16"))
    ...
    fun `a byte outside printable ascii is shown as a dot`() {
        val canvas = FakeCanvas()
        HexView().draw(canvas, byteArrayOf(0x41, 0x00, 0x7F), mapOf("bytesPerRow" to "16"))
    ...
    fun `columns line up across every row`() {
        val canvas = FakeCanvas()
        HexView().draw(canvas, ByteArray(64) { it.toByte() }, mapOf("bytesPerRow" to "16"))
    ...
    fun `a narrow panel still gets whole rows`() {
        ...
        HexView().draw(canvas, ByteArray(64) { it.toByte() }, emptyMap())
        val hexRows = canvas.texts.filter { it.color == ViewCanvas.DEFAULT_TEXT }
    ...
    fun `no data says so rather than drawing nothing`() {
        val canvas = FakeCanvas()
        HexView().draw(canvas, ByteArray(0), emptyMap())
    ...
    fun `a truncated value reports where it went wrong instead of failing`() {
        ...
        val canvas = FakeCanvas()
        Asn1View().draw(canvas, byteArrayOf(0x30, 0x0A, 0x02, 0x01), emptyMap())
    ...
    fun `random bytes do not take the view down`() {
        val canvas = FakeCanvas()
        Asn1View().draw(canvas, ByteArray(64) { (it * 37).toByte() }, emptyMap())
    ...
    fun `an image is drawn at the size its own header gives`() {
        ...
        ImageView().draw(canvas, png, emptyMap())
    ...
    fun `actual size never overflows a panel narrower than the image`() {
        ...
        ImageView().draw(canvas, qr("flow", moduleSize = "8"), mapOf("fit" to "actual"))
        val image = canvas.ops.filterIsInstance<FakeCanvas.Image>().single()
    ...
    fun `data that is not an image says so`() {
        ...
        val failure = assertFailsWith<IllegalStateException> {
            ImageView().draw(canvas, "not an image at all".encodeToByteArray(), emptyMap())
        }
