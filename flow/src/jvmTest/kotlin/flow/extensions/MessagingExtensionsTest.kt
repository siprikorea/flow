...
import flow.ext.slack.SlackExtension
import flow.ext.telegram.TelegramExtension
...
class MessagingExtensionsTest {
    ...
    private fun slack(vararg options: Pair<String, String>) = SlackExtension().process(
        mapOf("in" to "the report".encodeToByteArray()),
        mapOf("timeoutSec" to "10") + options,
    )["out"]!!.decodeToString()

    private fun telegram(vararg options: Pair<String, String>) = TelegramExtension().process(
        mapOf("in" to "the report".encodeToByteArray()),
        mapOf("timeoutSec" to "10") + options,
    )["out"]!!.decodeToString()
    ...
    fun `an empty input is refused rather than posted as an empty message`() {
        listOf(
            { SlackExtension().process(mapOf("in" to ByteArray(0)), mapOf("webhookUrl" to "http://x")) },
            { TelegramExtension().process(mapOf("in" to null), mapOf("botToken" to "t", "chatId" to "1")) },
        ).forEach { send ->
