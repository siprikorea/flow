...
private fun RootRow(ws: Workspace) {
    ...
    Row(
        ...
        Txt(ws.rootLabel, FlowType.bodyStrong, Palette.text, maxLines = 1)
        // full path of the folder being shown — greyed out, like IntelliJ's project root
        Txt(ws.dirLabel, FlowType.mono, Palette.faintText, modifier = Modifier.weight(1f), maxLines = 1)
    }
}
...
private fun ItemRow(ws: Workspace, row: Workspace.Row) {
    ...
    Row(
        ...
        if (row.isDir) {
            ...
            Txt(row.name, FlowType.body, if (selected) Palette.text else Palette.menuText, modifier = Modifier.weight(1f), maxLines = 1)
        } else {
            Spacer(Modifier.size(12.dp))
            Box(Modifier.size(14.dp), contentAlignment = Alignment.Center) {
                if (isFlow) KindBadge("f", if (isComp) Palette.catComponent else Palette.dimText, 13.dp)
                else Txt("\u25aa", FlowType.bodyStrong, Palette.faintestText)
            }
            Txt(
                if (isFlow) row.name.removeSuffix(".flow") else row.name,
                FlowType.body,
                when {
...
private fun MenuItem(
    ...
    Row(
        ...
        Txt(label, FlowType.body, Palette.text, modifier = Modifier.weight(1f))
        Spacer(Modifier.width(16.dp))
        if (shortcut.isNotEmpty()) Txt(shortcut, FlowType.small, Palette.dimText)
        if (submenu) Txt("\u25b6", FlowType.small, Palette.subText)
    }
}
