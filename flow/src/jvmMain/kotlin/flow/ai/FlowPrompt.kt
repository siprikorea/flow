...
internal object FlowPrompt {
    ...
    fun systemPrompt(projectRoot: String?): String = buildString {
        ...
        appendLine("  save_flow      — the same, written into the open folder; Flow opens it")
        appendLine()
        appendLine("Call list_nodes before building, so the ids and option values are the real ones")
        appendLine("rather than guesses.")
        appendLine()
        appendLine("When the user wants a flow, save_flow it — that puts it in the folder and Flow")
        appendLine("opens it. build_flow is for showing one without keeping it. Do not print a flow's")
        appendLine("contents for the user to copy: saving is what they asked for.")
        appendLine()
        appendLine("These tools are all you have here: nothing else writes files and nothing runs.")
        appendLine()
        if (projectRoot != null) {
