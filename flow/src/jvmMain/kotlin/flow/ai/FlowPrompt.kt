...
internal object FlowPrompt {
    ...
    fun systemPrompt(projectRoot: String?): String = buildString {
        ...
        appendLine("  start_flow     — start a flow's actual run, visible on screen")
        ...
        appendLine("the app', not just 'does it work' — that's set_flow_input then start_flow, not")
        appendLine("run_flow: run_flow never touches the app, it only computes headlessly and returns the")
        appendLine("result as text. set_flow_input puts a value in the real input port on the real open")
        appendLine("tab; start_flow then runs it there for real, the same as the user pressing Start")
        appendLine("themselves, and stop_flow is the same for Stop. These three, plus open_flow, need")
        appendLine("Flow itself running to have anything to act on — say so plainly if a call reports it")
        appendLine("isn't, rather than trying again the same way.")
        appendLine()
