...
// Install store: everything installed lives under modules/<id>/, holding either the module's
...
//
// modulesDir under the user's home is the only root. Nothing ships with the app: every module
...
internal object ModuleLoader {
    ...
    /**
     * What an installed module's jar is called.
     *
     * A jar underneath — the class loader reads it by content, not by name — but with a suffix of
     * its own so one is recognisable as a Flow module rather than as some library lying around.
     * The two older names are still scanned for, so nothing installed before this has to be
     * installed again.
     */
    const val MODULE_SUFFIX = ".flowmod"
    private val LOADABLE_SUFFIXES = listOf(MODULE_SUFFIX, ".flowext", ".jar")

    init {
        // The store has had three names: modules/ + components/ first, then one modules/, and
        // now modules/ again — the word the whole program uses. Each move is one rename of the
        // folder, and a name already taken is left alone rather than overwritten, so nothing
        // installed is ever silently replaced.
        runCatching {
            if (!modulesDir.exists()) {
                val previous = File(baseDir, "extensions")
                if (!(previous.isDirectory && previous.renameTo(modulesDir))) {
                    modulesDir.mkdirs()
                    listOf(previous, File(baseDir, "components")).forEach { old ->
                        old.listFiles { f: File -> f.isDirectory }?.forEach { dir ->
                            val target = File(modulesDir, dir.name)
                            if (!target.exists()) dir.renameTo(target)
                        }
                        // delete() only succeeds on an empty directory, so anything left behind by
                        // a name clash keeps its old home rather than being thrown away
                        old.delete()
                    }
                }
            }
    ...
    private fun ensureDirs() { modulesDir.mkdirs() }
    ...
    /* ───────── installed modules (a process per module) ───────── */

    // what a worker told us about one module it serves
    private class Loaded(val dir: File, val info: ModuleInfo)

    private val processes = LinkedHashMap<File, ModuleProcess>()

    /**
     * What Settings ▸ Modules has been set to, by module id.
     *
    ...
    private fun processFor(dir: File): ModuleProcess = synchronized(processes) {
        processes.getOrPut(dir) { ModuleProcess(dir, jarsIn(dir).toList()) }
    }

    // what a worker told us about one output it serves
    private class LoadedView(val dir: File, val info: ViewInfo)

    // Asks each module folder what it provides. A folder that cannot answer — a broken jar, a
    ...
    private fun scan() {
        ensureDirs()
        val modules = LinkedHashMap<String, Loaded>()
        ...
        modulesDir.listFiles { f -> f.isDirectory }?.forEach { dir ->
            ...
            runCatching { describe(proc) }.getOrDefault(emptyList()).forEach { modules[it.id] = Loaded(dir, it) }
    ...
    private fun describe(proc: ModuleProcess): List<ModuleInfo> {
    ...
    /** VIEW_DESCRIBE: the views one worker serves, which for most modules is none. */
    private fun describeViews(proc: ModuleProcess): List<ViewInfo> {
    ...
    // a worker's failure is this call's failure, carrying whatever the module said
    private fun ModuleProcess.Reply.orThrow(): DataInputStream {
        if (!ok) throw failure(payload)
        return DataInputStream(ByteArrayInputStream(payload))
    }

    /**
     * What the module said, without the framing it was written with.
     *
     ...
     */
    private fun failure(payload: ByteArray): ModuleFailure {
        ...
        val message = runCatching { Wire.readString(input) }.getOrNull()
            ...
            ?: return ModuleFailure(payload.decodeToString(), "")
        val type = runCatching { Wire.readString(input) }.getOrDefault("")
        return ModuleFailure(message, type)
    }
    ...
    /**
     * The settings an module offers for the values they currently hold.
     *
     * Asked rather than taken from what DESCRIBE said, because an module may answer it with
    ...
    fun listComponents(): List<String> {
        ensureDirs()
        return modulesDir.listFiles { f -> f.isDirectory && isComponentDir(f) }
            ?.map { "${it.name}.json" }?.sorted() ?: emptyList()
    }

    fun readComponent(name: String): String? {
        val id = name.removeSuffix(".json")
        return runCatching { File(modulesDir, "$id/$COMPONENT_FILE").takeIf { it.exists() }?.readText() }.getOrNull()
    }
    ...
    fun runComponent(id: String, inputs: Map<String, ByteArray?>): Map<String, ByteArray?> {
        ...
        // the component's bundled jars get their own worker, same as an installed module
        val proc = processFor(dir)
    ...
    // An module JAR provides modules only (components are built in the Flow tool).
    fun installJar(path: String, overwrite: Boolean): InstallResult {
        ...
        val probe = ModuleProcess(jar.parentFile ?: modulesDir, listOf(jar))
        // a jar may provide modules, views, or both — either is worth installing
        val ids = try {
        ...
        val conflicts = ids.filter { File(modulesDir, it).exists() }
        if (conflicts.isNotEmpty() && !overwrite) return InstallResult(conflicts = conflicts)

        ids.forEach { id ->
            release(File(modulesDir, id))
            val d = File(modulesDir, id).also { it.deleteRecursively(); it.mkdirs() }
            runCatching { jar.copyTo(File(d, "$id$MODULE_SUFFIX"), overwrite = true) }
        }
        invalidate()
