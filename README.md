...
- **Shortcuts** — the menu shows each item's binding; ⌘N / ⌘⇧N create a flow or folder, F2 renames and Delete deletes the tree selection (rename and delete apply while the tree is the panel last clicked in, so Delete still clears a canvas selection). All four are rebindable in Settings ▸ Keymap
