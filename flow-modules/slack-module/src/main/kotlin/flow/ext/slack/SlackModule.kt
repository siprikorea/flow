 ...
 */
class SlackModule : ModuleExtension {
    ...
    override val options = listOf(
        // blank takes the one set for the module, so a flow that names no channel still works
        // on a machine where one is configured
        ModuleOption("channel", OptionType.TEXT, ""),
        // the id of a message to reply under — the output of another Slack node
        ModuleOption("threadTs", OptionType.TEXT, ""),
        ModuleOption("timeoutSec", OptionType.NUMBER, "30"),
    )

    override val settings = listOf(
        ModuleOption("botToken", OptionType.TEXT, ""),
        ModuleOption("webhookUrl", OptionType.TEXT, ""),
        ModuleOption("channel", OptionType.TEXT, ""),
        ModuleOption("apiUrl", OptionType.TEXT, DEFAULT_API),
    )

    /** Both credentials. Named rather than flagged on the option — see ModuleExtension. */
