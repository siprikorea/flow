 ...
 */
class TelegramModule : ModuleExtension {
    ...
    override val options = listOf(
        // a numeric id, or @channelname for a public one. Blank takes the module's own.
        ModuleOption("chatId", OptionType.TEXT, ""),
        ...
        ModuleOption("parseMode", OptionType.SELECT, "", listOf("", "MarkdownV2", "HTML")),
        // the id of a message to reply to — the output of another Telegram node
        ModuleOption("replyTo", OptionType.TEXT, ""),
        // arrives without a sound, for something that is worth recording and not worth waking up for
        ModuleOption("silent", OptionType.SELECT, "false", listOf("false", "true")),
        ModuleOption("timeoutSec", OptionType.NUMBER, "30"),
    )

    override val settings = listOf(
        ModuleOption("botToken", OptionType.TEXT, ""),
        ModuleOption("chatId", OptionType.TEXT, ""),
        ModuleOption("apiUrl", OptionType.TEXT, DEFAULT_API),
    )
