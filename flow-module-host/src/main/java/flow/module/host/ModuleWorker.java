...
/**
 * Serves one module folder in its own process.
 *
 * Started with nothing on its classpath but the module contract, the Kotlin runtime and the
 * folder's own jars, so an module's dependencies cannot meet the app's — and the app's cannot
 * quietly replace the module's, which is what happened while both lived in one JVM.
 *
 ...
 * corrupt the stream — so the module's own stdout is redirected to stderr on the way in.
 */
public final class ModuleWorker {
    ...
    public static void main(String[] args) throws Exception {
        // an module printing to stdout would land in the middle of a frame
    ...
    private static Runnable readJob(
        ...
        switch (op) {
            ...
            case Wire.SETTINGS: {
                ...
                // module answer this by asking a server what it has
                return () -> reply(out, writeLock, reqId, o -> {
                    ModuleExtension e = need(byId, id);
            ...
            case Wire.VIEW_OPEN: {
                ...
                return () -> reply(out, writeLock, reqId, o -> {
                    ...
                    if (v == null) throw new IllegalStateException("view '" + id + "' is not in this install");
    ...
    private static void reply(DataOutputStream out, Object writeLock, int reqId, Body body) {
        ...
        try {
            ...
            // an module is arbitrary code: whatever it throws is this call's failure, not ours
            status = Wire.ERROR;
            buf.reset();
    ...
    private static ModuleExtension need(Map<String, ModuleExtension> byId, String id) {
        ...
        if (e == null) throw new IllegalStateException("module '" + id + "' is not in this install");
        return e;
    }
    ...
    private static void describe(Map<String, ModuleExtension> byId, DataOutputStream o) throws IOException {
        ...
        for (ModuleExtension e : byId.values()) {
            ...
            // the module's own settings, which the host edits once rather than per node, and
            ...
            // which group the palette lists it under; "" from an module built before categories
     ...
     *
     * Nothing an module writes is involved: a JVM knows its own windows, so the host can do this
    ...
    /**
     * The node's options over the module's settings.
     *
    ...
    /** What one install folder turned out to provide. */
    ...
    /**
     * Every jar named on the command line, loaded together as this one module.
     *
     * The kinds are looked up separately but come from the same loader, so one jar may ship more
     * than one — a module and the view that makes sense of what it produces.
     */
    ...
    private static Loaded load(String[] jars) throws Exception {
        ...
        URLClassLoader cl = new URLClassLoader(urls, ModuleWorker.class.getClassLoader());
        Loaded loaded = new Loaded();
        for (ModuleExtension e : ServiceLoader.load(ModuleExtension.class, cl)) {
            loaded.modules.put(e.getId(), e);
        }
        // and under the name a module was registered with before it was called one, so a jar built
        // against either contract loads without being rebuilt
        for (ProcessorExtension e : ServiceLoader.load(ProcessorExtension.class, cl)) {
            loaded.modules.putIfAbsent(e.getId(), e);
        }
