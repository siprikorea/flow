 ...
 *
 * The bytes are passed through as they are rather than re-encoded; all this output works out is how
 * big to draw them. The size comes from the image's own header, so the aspect ratio is the image's
 * and not a guess.
 */
class ImageOutput : OutputExtension {
    override val id = "flow.output.image"
    override val displayName = "Image Output"
    ...
    override fun draw(canvas: OutputCanvas, data: ByteArray, options: Map<String, String>) {
        ...
        canvas.text(
            grid.pad, grid.pad + drawHeight + 6f, caption, 11f, OutputCanvas.MUTED_TEXT, mono = true,
        )
