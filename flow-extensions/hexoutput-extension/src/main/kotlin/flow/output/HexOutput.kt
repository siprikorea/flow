 ...
 * columns is the tedious part of reading a dump, and it is the part the output can just do.
 */
class HexOutput : OutputExtension {
    override val id = "flow.output.hex"
    override val displayName = "Hex Output"
    ...
    override fun draw(canvas: OutputCanvas, data: ByteArray, options: Map<String, String>) {
        ...
        for (row in 0 until rows) {
            ...
            canvas.text(grid.x(0), y, offsetLabel(start), grid.fontSize, OutputCanvas.MUTED_TEXT, mono = true)
            ...
            canvas.text(grid.x(hexStart), y, hex.toString().trimEnd(), grid.fontSize, OutputCanvas.DEFAULT_TEXT, mono = true)
            canvas.text(grid.x(asciiStart), y, ascii.toString(), grid.fontSize, OutputCanvas.ACCENT, mono = true)
            ...
            if (selected != null && selected in start until end) {
                ...
                canvas.rect(
                    ...
                    OutputCanvas.ACCENT, filled = false,
                )
            }
        }
        ...
        if (selected != null) {
            ...
            canvas.text(grid.x(0), grid.y(rows + 1), note, grid.fontSize, OutputCanvas.ACCENT, mono = true)
        }
        truncationNote(shown, data.size)?.let {
            total += 2
            canvas.text(grid.x(0), grid.y(rows + 1), it, grid.fontSize, OutputCanvas.MUTED_TEXT, mono = true)
        }
        if (shown == 0) {
     ...
     *
     * An output keeps nothing between drawing and being clicked — it may not even be the same call on
